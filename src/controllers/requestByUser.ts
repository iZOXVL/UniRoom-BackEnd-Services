...
export const getRequestByUser = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const { status } = req.body; // Extraer el estado del cuerpo de la solicitud
    ...
    // Obtener los chats donde el usuario es participante y el estado coincide
    const chats = await db.chat.findMany({
      where: {
        participants: { has: userId },
        status: status, // Filtrar por el estado solicitado
      },
